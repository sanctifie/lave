...
export default function MealPayScreen() {
  const { mealOrderId, amount } = useLocalSearchParams<{ mealOrderId: string; amount: string }>();
  ...
  const startPolling = () => {
    ...
    pollRef.current = setInterval(async () => {
      pollCount.current++;
      try {
        const status = await paymentsService.getMealPaymentStatus(mealOrderId);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await paymentsService.initMealPayment({ mealOrderId, phoneNumber: phoneNumber.trim(), operator });
      setStep('pending');
      startPolling();
    } catch (e: any) {
  ...
  if (step === 'success') {
    return (
      <ScrollView contentContainerStyle={styles.resultContainer}>
        ...
        <Pressable style={styles.primaryBtn} onPress={() => router.replace('/(patient)/meals/orders' as any)}>
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <Text style={styles.pageTitle}>Payer la commande</Text>
        <Text style={styles.amountValue}>{amountFcfa.toLocaleString('fr-FR')} FCFA</Text>
        <Text style={styles.amountSub}>Commande repas</Text>
        ...
        <Text style={styles.secureNote}>🔒 Paiement sécurisé — libéré à la livraison</Text>
      </ScrollView>
    </KeyboardAvoidingView>
  );
}
