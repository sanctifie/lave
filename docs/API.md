...
| `POST /reviews` 🔒 | patient | Noter un service réellement utilisé ; le commentaire est modéré par MBOLO Assist (moteur rapide), un avis douteux est `flagged` |
...
| `POST /kyc/verifications/:type/:id/screen` 🔒 `[admin]` | admin | Pré-contrôle MBOLO Assist (moteur vision) |
